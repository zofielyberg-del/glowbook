            ...
            try {
                ...
                if (data.success) {
                    ...
                    const mappedProviders: Provider[] = data.salons.map((s: any) => ({
                        ...
                        status: s.subscription_status || 'unpaid',
                                ...
                                <div className="p-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50/30">
                                    ...
                                    <div className="flex gap-4">
                                        <div className="px-4 py-2 bg-white rounded-xl border border-gray-100 flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-bold text-gray-500">{providers.filter(p => ['active', 'canceling'].includes(p.status)).length} Aktiva</span>
                                        </div>
                                        <div className="px-4 py-2 bg-white rounded-xl border border-gray-100 flex items-center gap-2">
                                            <span className="w-2 h-2 bg-[#10b981] rounded-full"></span>
                                            <span className="text-xs font-bold text-gray-500">{providers.filter(p => p.status === 'trialing').length} Provperiod</span>
                                        </div>
                                        <div className="px-4 py-2 bg-white rounded-xl border border-gray-100 flex items-center gap-2">
                                            <span className="w-2 h-2 bg-amber-400 rounded-full"></span>
                                            <span className="text-xs font-bold text-gray-500">{providers.filter(p => p.status === 'unpaid').length} Obetalda</span>
                                        </div>
                                        <div className="px-4 py-2 bg-white rounded-xl border border-gray-100 flex items-center gap-2">
                                            <span className="w-2 h-2 bg-red-400 rounded-full"></span>
                                            <span className="text-xs font-bold text-gray-500">{providers.filter(p => p.status === 'canceled').length} Uppsagda</span>
                                        </div>
                                    </div>
                                </div>
                                                        ...
                                                        <td className="px-8 py-6">
                                                            <div className="flex items-center gap-3">
                                                                <span className={clsx(
                                                                    ...
                                                                    p.status === 'trialing' ? "bg-emerald-100 text-emerald-700" :
                                                                    p.status === 'unpaid' ? "bg-amber-100 text-amber-700" :
                                                                    p.status === 'canceling' ? "bg-orange-100 text-orange-700" :
                                                                    p.status === 'canceled' ? "bg-red-100 text-red-700" :
                                                                    "bg-red-100 text-red-700"
                                                                )}>
                                                                    {p.status === 'active' ? 'Aktiv' :
                                                                     p.status === 'trialing' ? 'Provperiod' :
                                                                     p.status === 'unpaid' ? 'Obetald' :
                                                                     p.status === 'canceling' ? 'Avslutas' :
                                                                     p.status === 'canceled' ? 'Uppsagd' :
                                                                     'Inaktiv'}
                                                                </span>
